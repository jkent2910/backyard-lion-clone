...
namespace :backyard_lion do
  ...
  task julie_send_coach_feedback_emails: :environment do
    spreadsheet_name = "julie_feedback_email"
    ...
    worksheet.rows.drop(1).each do |row|
      ...
      unless invite_attrs[:email].blank?
        InviteMailer.julie_coach_feedback(invite_attrs[:email], invite_attrs[:first_name], invite_attrs[:position], invite_attrs[:last_name], invite_attrs[:sport], invite_attrs[:college]).deliver_later
      end
